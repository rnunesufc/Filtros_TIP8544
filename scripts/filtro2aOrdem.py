...
wcF1 = 1/(R1*C1) #Frequencia de corte do filtro
KF=1 #filtro passivo
tauF1=1/wcF1 # constante de tempo do filtro 1a ordem

wcF2 = 1/(R2*C2) #Frequencia de corte do filtro
KF=1 #filtro passivo
tauF2=1/wcF2 # constante de tempo do filtro 1a ordem


print(f"--- Projeto do Filtro RC (2ª Ordem) ---")
print(f"Resistor: {R1} Ohms")
print(f"Capacitor: {C1} Farads")
print(f"Frequência de Corte (wc1): {wcF1:.2f} rad/s")
print(f"Frequência de Corte (fc1): {wcF1 / (2 * np.pi):.2f} Hz")
print(f"Constante de Tempo (tau1): {tauF1} s")
print(f"Resistor: {R2} Ohms")
print(f"Capacitor: {C2} Farads")
print(f"Frequência de Corte (wc2): {wcF2:.2f} rad/s")
print(f"Frequência de Corte (fc2): {wcF2 / (2 * np.pi):.2f} Hz")
print(f"Constante de Tempo (tau2): {tauF2} s")
...
HF1_control = ct.tf(numF1, denF1)
HF2_control = ct.tf(numF2, denF2)
HF_control = HF1_control*HF2_control
...
plt.suptitle(f'Filtro Passa-Baixa RC (2ª Ordem) - $\omega_c$={wcF1:.2f} rad/s', fontsize=14)
...
plt.axvline(wcF1, color='red', linestyle='--')
plt.plot(wcF1, -6, 'ro') # 'ro' = red circle
plt.text(wcF1 * 1.1, -8, f'$\omega_c$={wcF1:.2f} rad/s (-3 dB)', color='red')
...
plt.plot(wcF1, -90, 'ro')
plt.text(wcF1 * 1.1, -85, '-90°', color='red')
plt.show()
