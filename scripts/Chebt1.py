...
N=2;
wcF = 2000 #Frequencia de corte do filtro
rp=2 #riple = riple máximo 2db

# sig.cheby1(Ordem, Ripple, FreqCorte)
num, den = sig.cheby1(N, rp, wcF, btype='low', analog=True)
...
w0=np.sqrt(den[2]) #frequencia natuarl W0 do filtro (note que é diferente da freqeunca de corete WcF)
Q=w0/den[1]; #fator de qualidade w0/Q=2(zeta)Wo
zeta=den[1]/(2*w0)
K=num[0]/den[2]
...
#onde K = 1 + R3/R4 - > p K = 1 o ganho de realimentação direta é realizado 

# P/  R1=R2=R C1=10nF podemos obter R e C2 pois:


# W0 = ______1_______
#      R*sqrt(C1*C2)

# Q=  __sqrt(C1*C2)_____    -> (Q*2*C1)²=C1*C2
#          2*C1
         
C1=10e-6
C2=   ((Q*2)**2)*C1   
R=1/(w0*np.sqrt(C1*C2))   
         
R1=R # 1kohm
R2=R


#um vez que o ganho K original p topologia chebyshev havia dado menor que um tem-se que adicionar outra etapa p atenuação
#divisor de tensçao
...
print(f"--- Projeto do Filtro RC (2ª Ordem) ---")
print(f"Resistor R1 e R2: {R1} Ohms")
print(f"Capacitor C1 : {C1} Farads")
print(f"Capacitor C2 : {C2} Farads")
print(f"divisor de tensão:")
print(f"Resistor R5: {R5} Ohms")
print(f"Resistor R6: {R6} Ohms")

print(f"Frequência de Corte (wcF): {wcF:.2f} rad/s")
print(f"Frequência de Corte (fcF): {wcF / (2 * np.pi):.2f} Hz")
