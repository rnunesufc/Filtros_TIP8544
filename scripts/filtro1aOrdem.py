...
wcF = 1/(R*C) #Frequencia de corte do filtro
KF=1 #filtro passivo
tauF=1/wcF # constante de tempo do filtro 1a ordem

print(f"--- Projeto do Filtro RC (1ª Ordem) ---")
print(f"Resistor: {R} Ohms")
print(f"Capacitor: {C} Farads")
print(f"Frequência de Corte (wc): {wcF:.2f} rad/s")
print(f"Frequência de Corte (fc): {wcF / (2 * np.pi):.2f} Hz")
print(f"Constante de Tempo (tau): {tauF} s")
...
plt.suptitle(f'Filtro Passa-Baixa RC (1ª Ordem) - $\omega_c$={wcF:.2f} rad/s', fontsize=14)
...
plt.text(wcF * 1.1, -5, f'$\omega_c$={wcF:.2f} rad/s (-3 dB)', color='red')
...
plt.plot(wcF, -45, 'ro')
plt.text(wcF * 1.1, -40, '-45°', color='red')
plt.show()
