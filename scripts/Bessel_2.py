...
# projeto do arranjo Sallen-key 2a Ordem

# G = (K.W0²)/(s² +(W0/Q)s + W0²)

#p um butterworth de 2a ordem o polinomio caracteristico Bn(s) é dado por s² + 1.144s + 1
#onde a função de trasnferencia H(s) em sua forma genérica é dada por

#H(s) = ____________K*Wc²________________
#           s² + 2*(zeta)*Wc*s + Wc²

# zeta->Fator de amortecimento
# Wc -> Frequencia de corte do filtro

#no caso de maneira a atende o polinomio de 2a ordem p o filtro butterworh temos
#zeta = 0,707
# wc =2000 rad/s ou ~318Hz

#simplificando p o caso de resistores e capacitores iguais
#R1=R2=R e C1=C2=C

# W0 = __1___    e     Q =___1___
#       RC                 3 - K

#onde K = 1 + R3/R4

# P/  R4=10K -> R3=5.86k
# resulta no ganho desejado -> 1 + R3/R4 = 3 - 1/0,707 -> K=1.586
# p/ Wc=2000=1/RC -> p C=10nF -> R=50k
# --- 1. PROJETO TEÓRICO (Bessel) ---
N = 2
wcF = 2000.0  # Frequência (w0) definida pelos componentes R,C

# Alvo de Forma (Bessel N=2)
zeta = np.sqrt(3) / 2.0  # zeta = 0.866

# Ganho K (Exigido pela topologia Sallen-Key R,C iguais)
KF = 3.0 - 2.0 * zeta  # KF = 1.268

print(f"--- Filtro Bessel N=2 (Sallen-Key R,C Iguais) ---")
print(f"w0 (alvo) = {wcF:.2f} rad/s")
print(f"Zeta (alvo) = {zeta:.4f}")
print(f"Ganho K (necessário) = {KF:.4f}")

# --- 2. CÁLCULO DOS COMPONENTES (para o hardware) ---
# w0 = 1/(RC) = 2000
C = 10e-9 # 10nF
R = 1 / (wcF * C) # 1 / (2000 * 10nF) = 50k
R1 = R
R2 = R
C1 = C
C2 = C
...
# K = 1 + R3/R4 = 1.268
R4 = 10e3 # 10k
R3 = (KF - 1) * R4 # (1.268 - 1) * 10k = 2.68k
print(f"Componentes Sallen-Key (R,C iguais):")
print(f"R1=R2= {R1:.0f} Ohms")
print(f"C1=C2= {C1} Farads")
print(f"R4= {R4:.0f} Ohms")
print(f"R3= {R3:.2f} Ohms (Usar valor comercial 2.7k)")


# --- 3. DEFINIÇÃO DA FT (para a simulação) ---
# H(s) = (K * w0^2) / (s^2 + 2*zeta*w0*s + w0^2)
#
numF = [KF * wcF**2]
denF = [1, 2 * zeta * wcF, wcF**2]

# Criando os objetos de sistema
# Esta é a FT que será SIMULADA e CONSTRUÍDA
HF_control = ct.tf(numF, denF)
HF_scipy = sig.lti(numF, denF) 
...
plt.plot(wcF, 3, 'ro') # 'ro' = red circle
plt.text(wcF * 1.1, 3, f'$\omega_c$={wcF:.2f} rad/s (3 dB)', color='red')
